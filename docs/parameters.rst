...
.. Note::
    ...
    Several of these parameters and their values are created from ``incoming_data`` which is explained in the section `Pre-processing data for the model <https://east-africa-transport-risk-analysis.readthedocs.io/en/latest/predata.html>`_ 
...
If changes are made in the ``/data/hazards/floodmaps`` folder, execute the ``eatra.exposure.hazard_layers.sh`` script to update the ``hazard_layers_basic.csv``, ``hazard_layers.csv``, ``hazard_layers_chunks.csv``, and ``layers`` sub-folder.  
