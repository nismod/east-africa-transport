...
def main(config):
    ...
    # # Add road width
    # width = 3.25 # Default carriageway width in meters for Africa, needs to be generalizable for global
    ...
    # Tariff cost assignment from another data source. This one might be more realistic 
    edges_simple = gpd.read_file(os.path.join(data_path,"networks/road","roads.gpkg"), layer='edges', driver='GPKG')
    nodes = gpd.read_file(os.path.join(data_path,"networks/road","roads.gpkg"), layer='nodes', driver='GPKG')
    edges_simple.drop(["min_tariff","max_tariff"],axis=1,inplace=True)
    
    cost_data = pd.read_csv(os.path.join(data_path,"costs","road_tariffs.csv"))
    edges_simple = pd.merge(edges_simple,
                        cost_data[["iso_code","min_tariff","max_tariff"]],
                        how="left",left_on=["from_iso"],right_on=["iso_code"]).fillna(0)
    edges_simple.drop(["iso_code"],axis=1,inplace=True)
    edges_simple["tariff_cost_unit"] = "USD/ton-km"

    # Assign flow costs    
    time_cost_factor = 0.49
    edges_simple["min_flow_cost"] = (time_cost_factor*edges_simple["length_m"]/1000)/edges_simple["max_speed"] + (edges_simple["min_tariff"]*edges_simple["length_m"]/1000)
    edges_simple["max_flow_cost"] = (time_cost_factor*edges_simple["length_m"]/1000)/edges_simple["min_speed"] + (edges_simple["max_tariff"]*edges_simple["length_m"]/1000)
    edges_simple["flow_cost_unit"] = "USD/ton"

    print("Done adding cost attributes")

    gpd.GeoDataFrame(edges_simple,
                    geometry="geometry",
                    crs="EPSG:4326").to_file(os.path.join(data_path,"networks/road",
                                                                    "roads.gpkg"), layer='edges', driver='GPKG')

    nodes.to_file(os.path.join(data_path,"networks/road",
                                "roads.gpkg"), layer='nodes', driver='GPKG')               
