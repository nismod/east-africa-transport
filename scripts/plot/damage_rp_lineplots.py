...
def main(config):
    ...
    folder_path = os.path.join(figure_path,'damage_rp')
    ...
    for sector in sector_details:
        if sector['sector'] in ['road','rail']: # ['road', 'rail']
            damage_parquet = os.path.join(output_data_path,
                'risk_results',
            ...
            for hazard in hazards:
                ...
                if df.empty != True:
                    ...
                    save_fig(
                            os.path.join(
                                folder_path, 
                                f"{sector['sector_label'].lower().replace(' ','_')}_{sector['edge_layer']}_{hazard}_damage_lineplot.png"
                                )
                            )
                    plt.close()
